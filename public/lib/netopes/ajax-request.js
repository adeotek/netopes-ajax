...
const NAppRequest = {
	...
	sendRequest : function(targetId,action,property,content,loader,async,callback,callType,jsScripts) {
		...
		req.onreadystatechange = function() {
			if(req.readyState===4) {
				if(req.status===200) {
				    let actions = req.responseText.split(NAppRequest.actionSeparator);
                    ...
                    if(jsScripts) { NAppRequest.loadScripts(jsScripts); }
	...
	executeRequest : function(sessionId,requestId,targetId,action,property,loader,async,params,encrypted,jParams,eParam,postParams,callback,triggerOnInitEvent,callType,jsScripts) {
		...
		if(Array.isArray(jsScripts) && jsScripts.length>0) {
			for(let i=0;i<jsScripts.length;i++) {
				if(typeof(jsScripts[i])!=='object' || !jsScripts[i]['executeAfter']) { continue; }
				endJsScripts.push(jsScripts[i]);
				jsScripts.splice(i,1);
			}//END for
			NAppRequest.loadScripts(jsScripts);
		}//if(Array.isArray(jsScripts) && jsScripts.length>0)
	...
	executeRepeated: function(interval,targetId,action,property,params,encrypted,loader,async,triggerOnInitEvent,confirm,jParams,eParam,callback,postParams,sessionId,requestId,jsScripts,callType) {
		if(interval && interval>0) {
			...
			let execData = GibberishAES.enc(JSON.stringify({ targetId: targetId, action: action, property: property, params: GibberishAES.enc(params, 'xSTR'), encrypted: encrypted, loader: loader, async: async, triggerOnInitEvent: triggerOnInitEvent, confirm: confirm, jParams: jParams, eParam: eParam, callback: callback, postParams: postParams, sessionId: sessionId, requestId: requestId, jsScripts: jsScripts, callType: callType | 'executeRepeated' }),lTimer);
